    ...
    <div className="booking-page">
      ...
      <div className="booking-top-row">
        <div className="back-and-step">
          ...
          <span className="step-indicator">Крок 2/4</span>
        </div>
      </div>
          ...
          {timeSlots.length > 0 ? (
            timeSlots.map((slot, index) => (
              <button
                key={index}
               className={`time-slot ${
                  selectedTime?.start === slot.start_time && selectedTime?.end === slot.end_time ? "selected" : ""
                }`}
                onClick={() => setSelectedTime({ start: slot.start_time, end: slot.end_time })}
              >
